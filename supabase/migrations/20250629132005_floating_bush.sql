/*
  ...
  2. Status Column Enhancement
    - Convert from text to project_status enum
    - Update existing values to match enum format
    - Set proper default value

  3. New Tables
    - `project_custom_fields` - Flexible metadata storage for projects

  4. Security
    - Enable RLS on project_custom_fields table
...
-- Temporarily disable the trigger to avoid conflicts during updates
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Remove the existing default before changing type
